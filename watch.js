...
// ─── EMBED SOURCES ────────────────────────────────────────────────────────────
// vsembed.su URL formats:
//   Movie:   https://vsembed.su/embed/movie/{tmdb_id}
//   TV:      https://vsembed.su/embed/tv/{tmdb_id}/{season}/{episode}
const SOURCES = {
  vsembed: {
...
let totalSeasons   = 1;
let episodeCounts  = {}; // { season: count }
...
async function loadMovieDetails() {
  const [movie, credits] = await Promise.all([
    fetch(`${TMDB}/movie/${mediaId}?api_key=${API_KEY}`).then(r => r.json()),
    fetch(`${TMDB}/movie/${mediaId}/credits?api_key=${API_KEY}`).then(r => r.json()),
  ]);

  document.title = `${movie.title} — StreamVault`;
  renderDetails(movie, credits, "movie");
  loadPlayer();
}
...
async function loadTVDetails() {
  const [show, credits] = await Promise.all([
    fetch(`${TMDB}/tv/${mediaId}?api_key=${API_KEY}`).then(r => r.json()),
    fetch(`${TMDB}/tv/${mediaId}/credits?api_key=${API_KEY}`).then(r => r.json()),
  ]);

  document.title = `${show.name} — StreamVault`;
  totalSeasons = show.number_of_seasons || 1;

  // Build season/episode counts
  show.seasons.forEach(s => {
    if (s.season_number > 0) {
      episodeCounts[s.season_number] = s.episode_count;
    }
  });

  renderDetails(show, credits, "tv");
  buildSeasonSelect();
  buildEpisodeSelect();
  setupEpisodeNav();
  loadPlayer();
...
function buildEpisodeSelect() {
  ...
  sel.addEventListener("change", () => {
    currentEpisode = parseInt(sel.value);
    updateEpDisplay();
  });
}

function setupEpisodeNav() {
  document.getElementById("ep-load").addEventListener("click", loadPlayer);
...
function loadPlayer() {
  ...
  let url;
  if (type === "tv") {
    url = src.tv(mediaId, currentSeason, currentEpisode);
    updateEpDisplay();
  } else {
    url = src.movie(mediaId);
  }
  ...
  setTimeout(() => {
    ...
    // Fallback in case onload doesn't fire (cross-origin iframes)
    setTimeout(() => {
      loading.style.display = "none";
      iframe.classList.remove("hidden");
    }, 4000);
...
// ─── DETAILS ─────────────────────────────────────────────────────────────────
function renderDetails(data, credits, mediaType) {
  ...
  if (data.poster_path) {
    document.getElementById("detail-poster").src = `${IMG_BASE}w342${data.poster_path}`;
  }
...
async function loadSimilar() {
  const res  = await fetch(`${TMDB}/${type}/${mediaId}/similar?api_key=${API_KEY}&page=1`);
  ...
  items.forEach(item => {
    ...
    card.innerHTML = `
      <div class="card-img-wrap">
        <img src="${IMG_BASE}w342${item.poster_path}" alt="${title}" loading="lazy" />
...
function setupSearch() {
  ...
  input.addEventListener("input", () => {
    ...
    debounce = setTimeout(async () => {
      const res  = await fetch(`${TMDB}/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(q)}&page=1`);
      const data = await res.json();
      ...
      dropdown.innerHTML = results.map(item => {
        ...
        const p = `${IMG_BASE}w92${item.poster_path}`;
        return `<div class="sr-item" data-id="${item.id}" data-type="${item.media_type}">
          <img src="${p}" alt="${t}" /><div><strong>${t}</strong><span>${y}</span></div>
        </div>`;
      }).join("");
      ...
      dropdown.querySelectorAll(".sr-item").forEach(el => {
        el.addEventListener("click", () => {
          window.location.href = `movie.html?id=${el.dataset.id}&type=${el.dataset.type}`;
        });
      });
    }, 350);
  });
